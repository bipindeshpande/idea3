  ...
  return (
    <div className="rounded-3xl border border-default bg-surface p-8 shadow-soft">
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-2xl font-semibold text-primary">Payments ({payments.length})</h2>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-default">
              ...
              <th className="px-4 py-3 text-left font-semibold text-primary">Payment ID</th>
              <th className="px-4 py-3 text-left font-semibold text-primary">Date</th>
            </tr>
          </thead>
          <tbody>
            {payments.map((payment) => (
              <tr key={payment.id} className="border-b border-default hover:bg-surface">
                <td className="px-4 py-3 text-primary">{payment.user_email}</td>
                <td className="px-4 py-3 font-semibold text-primary">
                  ${payment.amount} {payment.currency}
                </td>
                <td className="px-4 py-3 text-secondary">{payment.subscription_type}</td>
                <td className="px-4 py-3">{getStatusBadge(payment.status)}</td>
                <td className="px-4 py-3 text-xs text-secondary font-mono">{payment.stripe_payment_intent_id}</td>
