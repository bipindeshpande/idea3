...
export function getCelebrationMessage(score) {
 if (score >= 9) {
 return { emoji: "🔥", message: "Your idea is on fire! Outstanding score!", color: "text-accent" };
 }
 if (score >= 8) {
 return { emoji: "🚀", message: "Your idea is looking solid! Great score!", color: "text-accent" };
 }
 if (score >= 7) {
 return { emoji: "✅", message: "Good foundation! Your idea has potential.", color: "text-accent" };
 }
 if (score >= 6) {
 return { emoji: "💪", message: "Potential detected! Refine and improve.", color: "text-accent" };
 }
