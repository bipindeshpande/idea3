...
export default function ProfileTab({ profile, onUpdate, getAuthHeaders, addToast }) {
 ...
 if (!profile && !editing) {
 return (
 <div className="text-center py-12 rounded-xl border border-default shadow-sm bg-surface p-6 md:p-7">
 <p className="text-primary text-primary leading-relaxed mb-4">Create your Founder Profile</p>
 <p className="text-sm text-secondary mb-6">Set up your profile to start connecting with other founders.</p>
 <button
 ...
 <h3 className="text-lg font-semibold text-primary flex items-center gap-2 mb-4">About You</h3>
 ...
 errors.full_name ? "border-default" : "border-default"
 }`}
 ...
 <div>
 <label className="block text-sm font-medium mb-2">
 Short Bio <span className="text-accent">*</span>
 </label>
 <textarea
 value={formData.bio}
 onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
 className={`w-full px-3 py-2 border rounded-lg ${
 errors.bio ? "border-default" : "border-default"
 }`}
 rows={4}
 placeholder="Tell us about yourself (minimum 20 characters)"
 />
 {errors.bio && <p className="text-xs text-accent mt-1">{errors.bio}</p>}
 </div>
 ...
 <div className="border-b border-default pb-6">
 <h3 className="text-lg font-semibold text-primary flex items-center gap-2 mb-4">What You Bring / What You're Looking For</h3>
 
 <div className="space-y-4">
 <div>
 <label className="block text-sm font-medium mb-2">
 Primary Skills <span className="text-accent">*</span>
 </label>
 <div className="flex flex-wrap gap-2 mb-2">
 {Array.isArray(formData.primary_skills) && formData.primary_skills.length > 0 ? (
 formData.primary_skills.map((skill, idx) => (
 <span key={idx} className="px-3 py-1 bg-surface text-accent rounded-full text-sm flex items-center gap-2">
 {skill}
 <button
 type="button"
 ...
 <span className="text-xs text-secondary italic">No skills added yet</span>
 )}
 </div>
 ...
 className="px-5 py-2.5 rounded-lg font-medium text-on-accent bg-accent hover:bg-accent-hover transition-all shadow-sm hover:shadow-md whitespace-nowrap"
 >
 Add
 </button>
 </div>
 ...
 <span className="ml-2 text-secondary">(Type a skill and press Enter or click Add)</span>
 </p>
 )}
 </div>
 ...
 <div>
 <label className="block text-sm font-medium mb-2">
 What You're Looking For <span className="text-accent">*</span>
 </label>
 <textarea
 value={formData.looking_for}
 onChange={(e) => setFormData({ ...formData, looking_for: e.target.value })}
 className={`w-full px-3 py-2 border rounded-lg ${
 errors.looking_for ? "border-default" : "border-default"
 }`}
 rows={3}
 placeholder="What kind of collaborators or opportunities are you looking for?"
 />
 {errors.looking_for && <p className="text-xs text-accent mt-1">{errors.looking_for}</p>}
 </div>
 ...
 {/* Privacy Notice */}
 <div className="bg-app p-4 rounded-lg border border-default">
