...
const ValidationReportPDF = ({ 
 ...
 const hasParameterAnalysis = parameterCards && parameterCards.some(c => c.details && c.details !== "See detailed analysis in the 'Detailed Analysis & Recommendations' tab for the complete breakdown.");
 
 let currentPage = 1;
 const totalPages = 1 + 
 (hasParameterAnalysis ? 1 : 0) +
 ...
 {/* Page 2: Parameter-by-Parameter Analysis */}
 {parameterCards && parameterCards.length > 0 && parameterCards.some(c => c.details) && (
 <Page size="A4" style={styles.page}>
 <View style={styles.header}>
 <Text style={styles.title}>Parameter-by-Parameter Analysis</Text>
 </View>

 <View style={styles.section}>
 {parameterCards.filter(c => c.details && c.details !== "See detailed analysis in the 'Detailed Analysis & Recommendations' tab for the complete breakdown.").map((card) => {
 const analysisParagraphs = formatTextToParagraphs(card.details);
 return (
 <View key={card.name} style={{ marginBottom: 16 }}>
 <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
 <Text style={[styles.groupTitle, { marginTop: 0 }]}>{card.name}</Text>
 <Text style={[styles.parameterScore, { color: getScoreColor(card.score) }]}>
 ...
 {/* Page 3: Detailed Analysis & Recommendations */}
 ...
 {/* Page 5: Final Validation Conclusion (if available) */}
