...
/**
 * Order interest areas based on startup_category for better UX
 */
export function orderInterestAreas(interestAreas, startupCategory) {
  if (!startupCategory || startupCategory === "both") {
    // For "both", show tech first, then non-tech
    const techAreas = interestAreas.filter(area => 
      TECH_INTEREST_AREAS.includes(area) || MIXED_INTEREST_AREAS.includes(area)
    );
    const nonTechAreas = interestAreas.filter(area => 
      NON_TECH_INTEREST_AREAS.includes(area)
    );
    return [...techAreas, ...nonTechAreas];
  }
  ...
  if (startupCategory === "tech") {
    ...
    const priorityOrder = [
      ...
      "Software / SaaS",
      "Education",
      "Finance / Accounting", // If it has tech subcategories
    ];
